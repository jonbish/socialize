...
class SocializeButtons {
    ...
    function get_services(){
        ...
        // Twitter
        $services['twitter']['official']['callback'][''] = SocializeButtons::twitter_official('short_url');
        $services['twitter']['official']['callback'][] = SocializeButtons::twitter_official();
        $services['twitter']['official']['js'] = array(
            'twitter-button' => 'http://platform.twitter.com/widgets.js'
        );
        $services['twitter']['topsy']['callback'] = SocializeButtons::twitter_topsy();
        $services['twitter']['tweetmeme']['callback'] = socializeWP::twitter_tweetmeme();
        
        // Allow plugin authors to hook into service array
        $services = apply_filters('socialize-services', $services);
        return $services;
    }
    ...
    public function __call($name, $args) {
        ...
        if(!preg_match('/^get/',$name))
            new WP_Error('Socialize: Invalid Method Syntax', __("A social sharing service was called with the wrong syntax."));
        
        // Grab service from method name
        $name = preg_replace('/^get/','',$name);
        
        // Check if service exists, otherwise throw error
        if(!isset($services[$name]))
            new WP_Error('Socialize: Invalid Service', __("A social sharing service was called that does not exist."));
        
        // Grab socialize settings
        if (!isset($socialize_settings)) {
            $socialize_settings = socializeWP::get_options();
        }
        
        // List of variables to replace in code
        $service_vars = array(
            'link' => get_permalink()
        );
        
        // Generate button code
        $buttoncode = $services[$name]['callback'];
    ...
    function twitter_official($switch){
            // Grab socialize settings
            if (!isset($socialize_settings)) {
                $socialize_settings = socializeWP::get_options();
            }
            
            // Switch between alternative methods
            switch($switch){
                case 'short_url':
            ...
            $buttonCode .= 'data-count="' . $socialize_settings['socialize_twitter_count'] . '" ';
    ...
    function createSocializeTwitter($service = "", $service_options = array(), $custom_settings = null) {
        global $post;
        $buttonCode = "";
        ...
        $socialize_settings = wp_parse_args( $custom_settings, $socialize_settings );
        ...
        switch ($service) {
            case "":
                ...
                $socialize_settings['socialize_twitterWidget'] = $socialize_settings['socialize_twitterWidget'];
                $socialize_settings['socialize_twitter_count'] = $socialize_settings['socialize_twitter_count'];
                $socialize_settings['socialize_tweetcount_via'] = $socialize_settings['socialize_tweetcount_via'];
                $socialize_settings['socialize_tweetcount_links'] = $socialize_settings['socialize_tweetcount_links'];
                $socialize_settings['socialize_tweetcount_size'] = $socialize_settings['socialize_tweetcount_size'];
                ...
                break;
            case "official":
                $socialize_settings['socialize_twitterWidget'] = $service;
                $socialize_settings['socialize_twitter_count'] = $service_options['socialize_twitter_count'];
                break;
            case "topsy":
                $socialize_settings['socialize_twitterWidget'] = $service;
        ...
        if ($socialize_settings['socialize_twitterWidget'] == "tweetmeme") {
			...
			<!-- 
				...
				tweetmeme_style = "' . $socialize_settings['socialize_tweetmeme_style'] . '";
				' . $tweetmeme_bitly . '
			//-->
			</script>
                        <script type="text/javascript" src="http://tweetmeme.com/i/scripts/button.js"></script>';
        } else if ($socialize_settings['socialize_twitterWidget'] == "topsy") {
            // Topsy button code
    ...
    function createSocializeBuffer($service = "", $service_options = array(), $socialize_settings = null) {
        ...
        switch ($service) {
            case "":
                ...
                $socialize_settings['socialize_tweetcount_via'] = $socialize_settings['socialize_twitter_source'];
                $buffer_counter = $socialize_settings['buffer_counter'];
                break;
            case "official":
                $socialize_settings['socialize_tweetcount_via'] = $service_options['socialize_twitter_source'];
        ...
        $buttonCode .= ' data-via="' . $socialize_settings['socialize_tweetcount_via'] . '"';
        if (has_post_thumbnail()) {
            $large_image_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID));
            $post_thumbnail = $large_image_url[0];
